const { getIdBdController } = require("../controlles/getIdBdController");

const getIdBdHandler = async (req, res) => {
    const { id } = req.params; // Obtiene el parámetro "id" de la solicitud

    try {
        const object = await getIdBdController(id);
        console.log(object)
        // Llama a la función getIdController pasando el ID como argumento para obtener el objeto correspondiente
